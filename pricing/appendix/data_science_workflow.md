# Data Science for Pricing — the technical workflow, end to end
...
`workspace/notes/` — into one workflow. It is the technical side of this track: everything a
...
Nothing in this workflow is new. Each step is a foundation idea wearing
...
|---|---|
| 1 · EDA | [File 2.1](../02_prove_the_change/01_averages_and_uncertainty.md)'s "averaged over whom?" + [The distributions appendix](distributions_for_pricing.md)'s shapes: look before you model |
| 2 · Conversion model | [File 1.3](../01_set_the_rate/03_elasticity.md)'s elasticity, measured per quote — the log-price coefficient **is** the elasticity |
| 3 · Expected profit | [File 1.1](../01_set_the_rate/01_price_cost_and_profit.md)'s contribution × [File 2.1](../02_prove_the_change/01_averages_and_uncertainty.md)'s expected value |
| 4 · GLMs | [The distributions appendix](distributions_for_pricing.md)'s shapes plugged into a regression |
| 5 · Guardrails | [File 4.1](../04_satisfy_the_regulator/01_segments_and_fairness.md)'s fairness line + [File 1.3](../01_set_the_rate/03_elasticity.md) §5's "a model asking for the cap is a warning" |
| 6 · Validation | [File 2.2](../02_prove_the_change/02_tests_and_experiments.md)'s "compared to what?", pointed at models — out-of-time is the control group *in time* |
| 7 · Explaining (SHAP) | [File 3.2](../03_defend_the_plan/02_kpis_and_stakeholders.md)'s stakeholders: a price nobody can explain is a price no committee will pass |
| 8 · Monitoring | [File 2.4](../02_prove_the_change/04_forecasting_and_stockouts.md)'s lesson that a system corrupts its own data — so keep a clean stream flowing |

## 1. Data and EDA (exploratory data analysis)

Before any model: look at the data. Histograms of every column ([The distributions appendix](distributions_for_pricing.md)'s
...
conversion model *contains* [File 1.3](../01_set_the_rate/03_elasticity.md)'s elasticity.
...
data were set *in response to* demand ([File 1.3](../01_set_the_rate/03_elasticity.md) §6, endogeneity), the model's
...
**45% of profit**. Fixes: experiments ([File 2.2](../02_prove_the_change/02_tests_and_experiments.md)) or instruments (glossary: IV).
...
right output shape ([The distributions appendix](distributions_for_pricing.md)) for the job. The table to recognise:
...
- **Decision drift** — are recommended prices creeping toward the cap?
  (A model asking for the cap is a warning, not an answer — [File 1.3](../01_set_the_rate/03_elasticity.md) §5.)
...
([File 1.3](../01_set_the_rate/03_elasticity.md)'s dividing line, spoken through the model.)
...
it's exactly the regime change [File 1.3](../01_set_the_rate/03_elasticity.md) warned makes elasticity a moving target.
...
[`../05_the_panel/N9_safe_exploration_consumer_duty.ipynb`](../05_the_panel/N9_safe_exploration_consumer_duty.ipynb),
