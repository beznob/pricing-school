...
This companion explains the thinking behind [omnichannel_one_pager.md](omnichannel_one_pager.md): the chain of reasoning a pricing manager runs through, the small amount of maths that carries the whole argument, and the concepts to learn (with pointers into the track files) so you can reproduce this reasoning on any similar question.
...
4. **Competitive positioning.** A single transparent rate is a differentiator against competitors who obfuscate (e.g. "0% commission" with a wide spread). See `../01_set_the_rate/05_competition.md`.
...
1. **Price elasticity of demand** — the % volume change per % price change; the single number the whole decision turns on. Know why elasticity differs by channel/segment (search costs, switching costs, salience of comparison). → `../01_set_the_rate/03_elasticity.md`
...
8. **Test-and-learn / causal inference** — pilots with control groups, difference-in-differences, why a before/after comparison is not evidence (FX volumes move with seasons, travel demand and the interbank rate). → `06_tests_and_experiments.md`, [file 2.3](../02_prove_the_change/03_causal_inference_primer.md)
