...
6.  Write figures to ``images/`` (``role_*.png``) and ``ROLE_RESULTS.md``.
...
def distribution_block(panel, iv):
    ...
    ax = axes[1]
    order = ["digital", "retail", "agency", "post_office", "tmc"]
...
def fair_value_block(portfolio):
    ...
    piv = piv.loc[["digital", "retail", "agency", "post_office", "tmc"]]
    ...
    out = generate_fair_value_assessment(portfolio, os.path.join(HERE, "FAIR_VALUE_ASSESSMENT.md"))
...
def write_results(tbl, pnl, pos, sc_fv, vh, lines, sc_aop, cs, ci):
    ...
    L += ["", "Digital is the most price-sensitive channel (price-transparent); TMC the "
    ...
    L += ["", "The same FX margin earns very different *net* contribution by channel: "
          ...
          "margin; inelastic channels (TMC, Post Office) should harvest.", "",
          ...
          "- See `FAIR_VALUE_ASSESSMENT.md` for the full documented assessment and "
          "remediation actions.", "",
    ...
    with open(os.path.join(HERE, "ROLE_RESULTS.md"), "w") as fh:
        fh.write("\n".join(L))
    print(f"\nWrote ROLE_RESULTS.md + 4 figures to images/.")
