...
Run `01_data_foundation.ipynb` first (it writes the CSVs), then:
...
# Every table notebook 01 writes, with the grain and why a pricing manager
# would open it. Keep this list in step with the `tables` dict in notebook 01.
TABLES: dict[str, str] = {
...
def build(data_dir: Path = DATA_DIR, db_path: Path = DB_PATH) -> Path:
    ...
    if missing:
        print(f"\n  ! not found (re-run notebook 01?): {', '.join(missing)}")
...
def main() -> None:
    ...
    parser.add_argument("--data-dir", type=Path, default=DATA_DIR,
                        help="folder holding the CSVs written by notebook 01")
