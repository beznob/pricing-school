# Explaining pricing charts in English
...
- **`FINANCE_FOR_PRICING.md`** — the finance behind every number quoted here.
- **`pricing_finance.py`** — the formulas, runnable, with doctests.
- **`db_helper.py`** — the queries the charts are built from.
- **`DATABASE_USAGE.md`** — the star schema and the SQL worth having.
