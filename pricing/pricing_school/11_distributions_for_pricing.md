# 11 · The Shapes of Randomness — which distribution for what

*(This file distils the repo's insurance-maths deep dive —
`probability_distributions_pricing.md` — into the six shapes a pricing person
actually uses. Skip the formulas on a first read; learn the **shapes and the
"use it when" rules**.)*

## 0. The bridge in — you already know why this file matters

Part 1 kept telling you that numbers lie in particular ways. This file gives
those ways names. If you remember the foundation on the left, the deep
version is one step away:

| You learned (Part 1) | This file makes it precise |
|---|---|
| The mean lies about money — one whale drags it (File 05 §1) | Money is **log-normal** (shape ②); mean ≫ median is your tail detector |
| The wobble decides how much data you need (File 05 §3) | Every shape carries its own wobble — and clumpy counts (④) wobble far more than Poisson admits |
| Counting customers arriving (File 09 §1) | Counts are **Poisson** (③) — or negative binomial (④) once real life clusters them |
| A sell-out day is a floor, not a measurement (File 09 §2) | Censoring = the top of a distribution cut off; fit shapes that respect the cut |
| Profit is squeezed between two big numbers (File 01 §4) | The squeeze breaks in the **tail** (⑥) — which is why you price the tail, not the average |
...
- **Tell-tale:** mean well above median (File 05's whale effect).
...
- **Use it when:** positive amounts with a moderate tail. It's the default
  *severity* family in insurance GLMs (File 12).
...
- **Pricing lesson:** if losses are Pareto-ish, "average cost" is a
  dangerous number; you price the tail (and cap it — see File 12 on capping).
...
| A customer converts or not | Bernoulli (a coin with probability p) | the atom of conversion modelling (File 12) |
...
**hedge** in the glossary, File 10; the runbook notebook N11 opens with the
hedge book).
...
[`../probability_distributions_pricing.md`](../probability_distributions_pricing.md)
(all formulas, goodness-of-fit, extreme-value theory, Tweedie/Beta/Weibull,
tail-risk measures, credibility). Every simulator in
[`../fres_prep/`](../fres_prep/README.md) uses these shapes — now you'll
recognise them.
