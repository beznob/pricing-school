# 08 · Competition — price wars and how not to start one
...
2. **Targeted price moves** — cut only where it pays: the specific branches
   ...
   give (Files 01+03: contribution and elasticity decide, branch by branch).
...
their paths (this is the control-group logic of File 06, built by matching
...
Why those 20 and not all 50? (Two files from this school answer it.)
...
(File 03) and margin (File 01). At inelastic branches, cutting is pure
...
2. **Files 01 + 03 decide *where* to fight:** a defensive cut only pays at
   branches where the contribution given up (File 01) is smaller than the
   volume defended — which each branch's elasticity (File 03) decides. The
   response ladder's rung 2 is those two files doing local arithmetic.
3. **File 06's logic, without the randomiser:** you can't A/B-test a
   ...
   **matched twins** (§5). The glossary's *synthetic control* is the formal
   ...
   that didn't get hit.
4. **File 12 §2** is the always-on version of the war: on comparison sites
...
5. **In code:**
   [`../fres_prep/N8_competitor_response_game.ipynb`](../fres_prep/N8_competitor_response_game.ipynb)
