# 02 · Percentages, Basis Points, and Where the FX Price Hides
...
Remember tiers — they come back in File 04 as a place where money leaks
...
2. **File 04** does business in these units: the price waterfall's ledges
   are measured in bps, and a leakage audit is a hunt for the bps that went
   missing between the rate board and the till.
3. **File 07** is the same total-cost comparison with a regulator watching.
...
4. **File 11 §4** names the danger underneath the spread: daily rate moves
   have fatter tails than a bell curve, which is why the **hedge** (see the
   glossary, File 10) exists — the spread you earn must survive the day the
   market jumps.
5. **The real numbers:** the master prompt's Part 1 has the market's
