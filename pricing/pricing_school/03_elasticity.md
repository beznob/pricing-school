# 03 · Elasticity — how customers react when you change the price
...
everyone is almost always wrong** — more in Files 07 and 08.
...
demand you're trying to measure). File 06 explains the honest fixes; the
...
gain. (But check fairness — File 07 — and competition — File 08 — first.)
...
2. **The honesty rung (File 05):** any measured elasticity is an *estimate*
...
3. **The trap rung (File 06):** the estimate only means something if the
...
4. **The grown-up form (File 12 §2):** in production, elasticity lives
...
5. **The payoff rung (File 12 §3):** the markup rule (§5) is the shortcut
...
6. **In code:**
   [`../fast_transaction_services/elasticity_models.py`](../fast_transaction_services/elasticity_models.py)
   ...
   **45% of profit**, the case study's headline result. Then
   [`../fres_prep/N2_hierarchical_elasticity.ipynb`](../fres_prep/N2_hierarchical_elasticity.ipynb)
