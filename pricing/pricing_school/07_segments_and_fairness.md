# 07 · Different Prices for Different People — and the fairness line

## 1. Segments: customers are not one blob

A **segment** is a group of customers who behave similarly: how much they
buy, how price-sensitive they are (File 03), which channel they use.
...
by segment (File 03's markup rule). Charging everyone the same price
...
Here's the subtle bit — the single most useful fairness insight in this
school. Suppose your price sheet is purely channel-based:
...
(File 04) creating a fairness problem — and it is invisible in averages and
...
1. **Built from three rungs below:** segment elasticities (File 03 §4)
   explain *why* prices differ by group; the mix effect (File 04 §2)
   explains how a fair-looking price sheet can land unfairly; and "averages
   hide groups" (File 05 §2) is the habit that catches it. The §3 trap is
   literally File 04's mix effect caught doing something the regulator cares
   about.
2. **File 12 §5** wires the fairness line into the machinery: price caps and
...
3. **File 13** makes it a standing duty rather than a one-off analysis:
...
4. **In code:**
   [`../fres_prep/N6_channel_differential_fair_value.ipynb`](../fres_prep/N6_channel_differential_fair_value.ipynb)
   ...
   [`../fast_transaction_services/fair_value.py`](../fast_transaction_services/fair_value.py)
