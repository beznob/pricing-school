# 3.3 · Running the Function — Operating Model, P&L, AOP, Regulation, Governance

*(Consolidated from the former `pricing_manager_handbook` chapters 00, 09, 10,
11, 17, 18 and 19. Foundations: KPIs & stakeholders in [File 3.2](02_kpis_and_stakeholders.md), fairness in
[File 4.1](../04_satisfy_the_regulator/01_segments_and_fairness.md), monitoring in [The workflow appendix](../appendix/data_science_workflow.md) §8.)*
...
lever of [File 1.4](../01_set_the_rate/04_price_volume_mix_and_leakage.md). Core formulas: CM ratio `(P−V)/P`; break-even volume
`fixed ÷ (P−V)`; discount break-even `d/(CM−d)` and rise tolerance `d/(CM+d)`
([File 1.7](../01_set_the_rate/07_channels_promotions_revenue_management.md) §2). High operating leverage (fixed-heavy) makes profit very sensitive
...
under-perform targeted ones** ([File 1.7](../01_set_the_rate/07_channels_promotions_revenue_management.md) §1), and the scenario engine is how you
prove it to Finance.
...
with board-level MI**. [File 4.1](../04_satisfy_the_regulator/01_segments_and_fairness.md) teaches the mix trap that makes group averages
lie.
...
**Monitoring (MLOps for pricing — [The workflow appendix](../appendix/data_science_workflow.md) §8 is the foundation):** PSI on
...
[File 3.2](02_kpis_and_stakeholders.md) carries the core set. Additional metrics worth defining precisely:
...
[File 3.2](02_kpis_and_stakeholders.md) §§2–4 carry the stakeholder map and committee playbook. Additions from
...
- **30-60-90:** learn the P&L/data/competitors/conduct posture and find the
  ...
  [File 3.2](02_kpis_and_stakeholders.md) §5.)
...
1. The draft plan has margin up 10% and volume flat on the digital line. What is
   wrong with it and how do you fix it?
2. Revenue is £2m below plan. Write down the bridge you would build and say what
   each line means.
3. One colleague suggests sharing next quarter's planned margins with a rival's
   pricing team "to keep the market orderly". Another wants to set a minimum
   resale price for a partner. What is at stake in each case?

**Answers.** (1) It is internally inconsistent. Digital is the most elastic
channel, at around minus 2 in the case study, so a 10% margin rise implies a
volume fall of roughly a fifth, not zero. Run the line through the elasticity so
volume responds, and present base, upside and downside rather than a point. (2)
Change in revenue equals the volume effect (change in volume times the old price)
plus the price effect (change in price times the new volume) plus the mix effect
(share shifting between high and low margin lines), plus FX and competitor
effects where they apply. Pick a convention and keep it. If you cannot bridge the
gap, you do not yet understand it. (3) Sharing future pricing intentions is an
information exchange under Chapter I of the Competition Act and can be illegal
even without an agreement. Dictating a partner's minimum resale price is resale
price maintenance, also generally unlawful; a maximum or a recommended price is
fine.
