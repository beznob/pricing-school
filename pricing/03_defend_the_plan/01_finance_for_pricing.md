# 3.1 · The finance a pricing manager is expected to know

Every formula here is implemented, with a worked example, in
[`../engine/pricing_finance.py`](../engine/pricing_finance.py). Read this for the reasoning; run
...
`../02_prove_the_change/lab_01_data_foundation.ipynb`). Nothing here is illustrative-only.
...
spread. That is why "one price across all channels" ([the omnichannel lab](../05_the_panel/lab_omnichannel_case_study.ipynb)) is a hard
...
2. **A hurdle is not a decision.** It tells you what it would take. Only an
   *estimate of demand* tells you whether you clear it — which is why the next
   section exists, and why [lab 1.2](../01_set_the_rate/lab_02_demand_and_elasticity.ipynb), [lab 2.3](../02_prove_the_change/lab_03_regression_for_pricing.ipynb), [lab 2.4](../02_prove_the_change/lab_04_experiments_and_ab_testing.ipynb) and [lab 2.5](../02_prove_the_change/lab_05_causal_inference.ipynb) are all about getting
...
- **Watch how violently it moves.** Between elasticity −2.5 and −1.4 the
  ...
  than its point value, which is why [lab 1.3](../01_set_the_rate/lab_03_price_optimisation.ipynb) optimises over a distribution
...
Nothing about the arithmetic is at fault; the *correlation is not the effect*. Every technique in [lab 2.4](../02_prove_the_change/lab_04_experiments_and_ab_testing.ipynb) and [lab 2.5](../02_prove_the_change/lab_05_causal_inference.ipynb) —
...
Meridian's actual behavioural rule, the one [lab 2.1](../02_prove_the_change/lab_01_data_foundation.ipynb) simulates and [lab 1.3](../01_set_the_rate/lab_03_price_optimisation.ipynb) has to rediscover:
...
| To estimate demand honestly | §4 | [lab 1.2](../01_set_the_rate/lab_02_demand_and_elasticity.ipynb), [lab 2.3](../02_prove_the_change/lab_03_regression_for_pricing.ipynb), [lab 2.4](../02_prove_the_change/lab_04_experiments_and_ab_testing.ipynb) and [lab 2.5](../02_prove_the_change/lab_05_causal_inference.ipynb) |
| To explain a number to a board | §5 | `db.price_waterfall()`, `db.revenue_bridge()` |
| To price for the long run | §6 | [lab 1.3](../01_set_the_rate/lab_03_price_optimisation.ipynb), `db.retention_by_price()` |
| The whole job at once | — | **[the omnichannel lab](../05_the_panel/lab_omnichannel_case_study.ipynb)** |
...
1. Price £100, variable cost £60. What is the margin, what is the markup, and
   which one does the discount formula use?
2. With a contribution margin of 40%, a 5% price cut needs what volume uplift to
   stand still, and a 5% price rise can afford what volume loss?
3. A static optimiser says raise the margin by 30 basis points. What correction
   makes that answer honest, and which way does it push the number?

**Answers.** (1) Margin is 40 over 100, so 40%. Markup is 40 over 60, so 67%. The
discount break-even uses the contribution margin ratio, 40%. Mixing the two is
the most common arithmetic error in a pricing meeting. (2) The cut needs 0.05
divided by (0.40 minus 0.05), which is a 14.3% uplift. The rise can afford 0.05
divided by (0.40 plus 0.05), which is an 11.1% loss. (3) The lifetime value
correction. Charging above the market costs future visits, so the true cost of a
rise includes the repeat business it loses, not only the orders lost today. It
pushes the optimal price down, which is why every static optimiser is too
aggressive until it is corrected.
