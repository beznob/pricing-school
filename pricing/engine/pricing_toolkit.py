"""Shared plumbing for the runnable lessons (the `lab_*` notebooks).
...
1. finds the `data/` folder (built by lab 2.1) no matter where the
...
def load(name: str) -> pd.DataFrame:
    ...
    for col in df.columns:
        if col in DATE_COLS:
            ...
            # Plain object, not the "string" extension dtype. Under pandas 3 that dtype
            # is arrow-backed, so `df["product_id"].eq("TRAVEL_CARD") * price` raises
            # instead of treating the mask as 0/1. Ids are compared and grouped on, never
            # text-processed, so object costs nothing and keeps the masks arithmetic.
...
def truth() -> dict:
    """The answer key written by lab 2.1 — the parameters that generated the world."""
    path = data_dir() / "truth.json"
    if not path.exists():
        raise FileNotFoundError(f"Could not find {path}. Run ../02_prove_the_change/lab_01_data_foundation.ipynb first.")
