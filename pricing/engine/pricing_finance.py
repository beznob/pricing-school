...
`../02_prove_the_change/lab_01_data_foundation.ipynb`), so the numbers you see here are the numbers the
...
See `../03_defend_the_plan/01_finance_for_pricing.md` for the same ground in prose, with the traps.
"""
...
def contribution_margin(price: float, variable_cost: float) -> float:
    ...
    whole reason "one price across all channels" (the omnichannel lab) is a hard
...
# the same demand you are trying to measure. Lab 1.2, lab 2.3 and lab 2.5 are
...
def arc_elasticity(p0: float, q0: float, p1: float, q1: float) -> float:
    ...
    demand was strongest. The instrument in lab 2.5 exists to fix exactly
    this.
    """
    if p1 == p0:
...
def semi_elasticity(q0: float, q1: float, margin_change_pp: float) -> float:
    ...
    changes when you move the price. That is lab 4.1's subject.
    """
...
def volume_after_price_change(q0: float, pct_price_change: float, elasticity: float) -> float:
    ...
    10%. For a bigger move, project from the demand curve itself (lab 1.2) rather than from a single slope, because elasticity is not constant:
...
def optimal_price_from_elasticity(marginal_cost: float, elasticity: float) -> float:
    ...
    matters more than its point value. This is why lab 1.3 optimises over
...
def retention_multiplier(margin_above_market_pp: float, refused_for_stock: bool = False,
    ...
    simulates and lab 1.3 has to rediscover:
    ...
    Note what that second number means commercially. Lab 1.3's static
...
def lifetime_value_of_a_price_move(
    ...
    back — that is why lab 1.3's optimiser needs the correction — but in
...
def _tour() -> None:
    ...
    print("  board: lab 4.1 is about whether you are allowed to split it.")
