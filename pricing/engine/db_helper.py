...
CSVs from `../02_prove_the_change/lab_01_data_foundation.ipynb`).
...
"the price cut caused the volume" — is what lab 2.4 and lab 2.5 are for, and
...
DEFAULT_DB = Path(__file__).resolve().parents[1] / "data" / "meridian.db"
...
class MeridianDB:
    ...
    def __init__(self, db_path: str | Path | None = None):
        ...
        if not self.db_path.exists():
            raise FileNotFoundError(
                ...
                "(which needs the CSVs from ../02_prove_the_change/lab_01_data_foundation.ipynb — run that first)."
            )
    ...
    def unit_economics(self, group_by: str = "channel") -> pd.DataFrame:
        ...
        Read it across a row and the structure of the business appears:
          ...
          * **online** — the same £509 order and almost the same spread as
            ...
            channels" (the omnichannel lab, 05_the_panel/lab_omnichannel_case_study.ipynb) is a real question and not a tidy-up.
    ...
    def discount_hurdles(self, discount_pct: float = 0.10,
        ...
        (lab 1.2) and ask whether the required uplift is achievable. If
    ...
    def margin_by_segment(self) -> pd.DataFrame:
        ...
        *sign*. That confound is lab 2.5's opening example.
        """
        return self.query(f"""
            SELECT q.segment,
    ...
    def stockout_cost(self) -> pd.DataFrame:
        ...
        Lab 1.4 (01_set_the_rate/lab_04_competition_and_dynamics.ipynb) recovers the uncensored demand curve properly.
        """
        return self.query("""
    ...
    def retention_by_price(self, buckets: int = 5) -> pd.DataFrame:
        ...
        differently for reasons that have nothing to do with price. Lab 2.1 does the like-for-like comparison properly, by normalising each
